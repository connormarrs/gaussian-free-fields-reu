...
class DFGF_S1 {
    private:
        ...
        //vector<double> z_vars;

    public:
        DFGF_S1(double s);
		DFGF_S1() = default;
        double eigval(int r, int n);
        double eigfcn(int r, int j, int n);
        double dfgf(int k, int n, double s, vector<double> z_vars);
        double compute_max();
		vector<double> FullDFGF_S1(double s, int n, vector<double> randomVector);
		vector<vector<vector<double>>>  parallelDFGF(double s, int start_in, int end_in, int num_in, int size, int num_seqs);
		vector<vector<double>> parallelMax(vector<vector<vector<double>>>);
		vector<double> parallelMeans(vector<vector<double>> data);
