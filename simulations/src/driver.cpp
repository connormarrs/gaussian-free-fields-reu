...
double sampleDFGF(int nvalue, double svalue, int numTrials, RandVec randvec) {
...
int main() {
	...
	// Creates a vector of tasks for threading
	...
	for(long unsigned int n_index=0; n_index<n_vals.size(); n_index++) {
		for(long unsigned int s_index=0; s_index<s_vals.size(); s_index++){
			jsonRoot["info"]["n"][to_string(n_vals[n_index])]["s"][to_string(s_vals[s_index])]["meanOfMaxima"] = tasks.front().get();
			tasks.pop();
